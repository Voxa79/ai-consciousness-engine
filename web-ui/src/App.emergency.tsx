...
const DiagnosticPage: React.FC = () => {
  ...
  useEffect(() => {
    // Intercepter les connexions WebSocket en DEV uniquement et éviter l'erreur TS en utilisant une sous-classe typée
    ...
    class PatchedWebSocket extends originalWebSocket {
      constructor(url: string | URL, protocols?: string | string[]) {
        ...
        const isBlocked = urlString.includes('127.0.0.1:3001') || urlString.includes('localhost:3001');
        if (isBlocked) {
          errorCount++;
          setWsErrors(errorCount);
          console.warn('🚫 WebSocket bloqué:', urlString);
        }
        // Appeler le constructeur d'origine
        super(url as any, protocols as any);
        // Si bloqué, fermer immédiatement la connexion pour la neutraliser en DEV
        if (isBlocked) {
    ...
    // Affectation typée pour satisfaire TS
